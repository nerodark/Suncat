            ...
            if (GetRawInputDeviceList(IntPtr.Zero, ref deviceCount, (uint)dwSize) == 0)
            {
                ...
                for (int i = 0; i < deviceCount; i++)
                {
                    ...
                    if (pcbSize > 0)
                    {
                        ...
                        if (deviceName.ToUpper().Contains("ROOT") || deviceName.Length == 0) // || !deviceName.StartsWith("\\\\?\\HID") || !deviceName.StartsWith("\\\\?\\ACPI"))
                        {
                            continue;
                        }
        ...
        public void ProcessInputCommand(Message message)
        {
            ...
            try
            {
                                     ...
                                     (uint)Marshal.SizeOf(typeof(RAWINPUTHEADER))) == dwSize)
                {
                    ...
                    // RAWINPUT32 OR RAWINPUT64
                    // DON'T FORGET TO ALSO REPLACE ToInt32() OR ToInt64() IN EnumerateDevices TO MATCH THE GOOD BITNESS
                    RAWINPUT64 raw = (RAWINPUT64)Marshal.PtrToStructure(buffer, typeof(RAWINPUT64));
